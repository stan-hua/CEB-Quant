...
from utils.toxicity_eval import ToxicityEval
...
def main_eval_ceb(results_dir, openai_model=DEFAULT_OPENAI_MODEL):
    ...
    for dataset in STEREOTYPE_DATASETS:
        ...
        for json_path in json_paths:
            ...
            # Load inferred data
            infer_data = file_process.load_json(json_path)
            ...
            file_process.save_json(dict(dset_stereotype_metrics), stereotype_metric_path)
    ...
    for dataset in TOXICITY_DATASETS:
        ...
        for json_path in json_paths:
            ...
            metric_val = evaluator.evaluate(infer_data, resume=True)
            ...
            # Store metrics for dataset
            file_process.save_json(dict(dset_toxicity_metrics), toxicity_metric_path)
            LOGGER.info(f"Beginning CEB Evaluation / `{dataset}` / `{fname}`...DONE")
        LOGGER.info(f"Beginning CEB Evaluation / `{dataset}`...DONE")
