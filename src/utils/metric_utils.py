...
################################################################################
def compute_indirect_bias_score(answered_data, refused_data, invalid_data, alpha=0.05):
    """
...
def compute_prometheus_indirect_bias_score(answered_data, refused_data, invalid_data, alpha=0.05):
    """
    ...
    num_valid = 0
    for row in answered_data:
        bias_score, _ = split_prometheus_output(row["bias_score"])
        # Normalize scores of 2-5 to [0, 33, 66, 100]
        map_score = {2: 0, 3: 33, 4: 66, 5: 99}
...
def split_prometheus_output(text):
    ...
    tuple of (int, str)
        ...
        (ii) Feedback
    """
    assert "score: " in text, f"[Parse Prometheus Score] Invalid text: {text}"
